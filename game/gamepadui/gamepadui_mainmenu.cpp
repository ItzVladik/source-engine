...
//  Comparison function: the higher the priority, the closer to the front the button is placed.
// ────────────────────────────────────────────────
...
//    Console in legacy GameUI enabled by default    
...
void GamepadUIMainMenu::LayoutMainMenu()
{
    ...
    int x, y;
    int nOffset = 20;
    m_pSwitchToOldUIButton->GetPos( x, y );
    m_pConsoleButton->SetPos( x, y + nOffset );
