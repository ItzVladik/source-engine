...
// Purpose:
//-----------------------------------------------------------------------------
void CFlashlightEffect::InitTexture()
{
	KeyValues *pMat = new KeyValues( "Modulate" );
	pMat->SetInt("$znearer", 1);
	pMat->SetString("$basetexture", "ezero/mask_nvmap");
	pMat->SetInt("$translucent", 1);
	pMat->SetInt("$additive", 1);
	pMat->SetInt("$mod2x", 1);

	PrecacheMaterial( "ezero/mask_nvmap" );

	m_CoverScreenTexture.Init( "ezero/mask_nvmap", TEXTURE_GROUP_OTHER, pMat );
}
...
void CFlashlightEffect::TurnOn()
{
	...
	if ( IsNVG() )
	{
		view->SetScreenOverlayMaterial( m_CoverScreenTexture );
		CLocalPlayerFilter filter;
		//engine->ClientCmd( "Ez_Nvg_On" ); //Breadman - see View_scene.cpp for this Convar
	}
#endif
